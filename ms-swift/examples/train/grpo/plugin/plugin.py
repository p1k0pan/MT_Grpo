...
class CometRMPlugin(ORM):

    def __init__(self):
        print("load comet")
        # model_path = download_model("Unbabel/wmt23-cometkiwi-da-xl")
        self.model = load_from_checkpoint("/mnt/workspace/xintong/pjh/models/wmt23-cometkiwi-da-xl/checkpoints/model.ckptl")
        import logging
        loggers = [logging.getLogger(name) for name in logging.root.manager.loggerDict]
        for logger in loggers:
            logger.setLevel(logging.WARNING)
    
    def format(self, completions):
        """Reward function that checks if the completion has a specific format."""
        pattern = r'^<think>.*?</think>\s*<translate>.*?</translate>(?![\s\S])'
        matches = [re.match(pattern, content, re.DOTALL | re.MULTILINE) for content in completions]
        return [1.0 if match else -1.0 for match in matches]
    
    def bleu(self, refs, mts, lang):
        if lang == 'zh':
            tokenize = "zh"
        elif lang == "en":
            tokenize = "13a"
        bleu = sacrebleu.sentence_bleu(mts, [refs], lowercase=True, tokenize=tokenize)
        return bleu.score
    ...
    # def comet_score_no_trainer(
    #     self,
    #     samples,                     # List[dict]  每条含 src / mt / ref
    #     batch_size=8,
    #     device="cpu",                # 或 f"cuda:{torch.cuda.current_device()}"
    ...
    #     # —— 1. 可选长度排序，减少 padding —— #
    #     if length_batching:
    #         key = "src" if "src" in samples[0] else "ref"
    #         sort_ids = sorted(range(len(samples)), key=lambda i: len(samples[i][key]))
    #         samples_sorted = [samples[i] for i in sort_ids]
    #     else:
    #         sort_ids = None
    ...
    #         sampler=SequentialSampler(samples_sorted),
    #         collate_fn=self.model.prepare_for_inference,  # 官方同款
    #         num_workers=0,                           # 简单最安全
    #     )
    ...
    #             batch = self._move_to_device(raw_batch, device)

    #             # ---------- 关键分支：dict vs tuple ----------
    #             if isinstance(batch, dict):
    #                 pred = self.model(**batch)              # COMET 1.x
    #             elif isinstance(batch, (list, tuple)) and isinstance(batch[0], dict):
    #                 pred = self.model(**batch[0])           # COMET 2.x+, 第 0 个元素才是输入
    #             else:                                  # 极少数老分支：纯 tensor 序列
    #                 pred = self.model(*batch)
    #             # ---------------------------------------------

    #             scores.extend(pred.score.cpu().tolist())

    #     # —— 2. 还原原始顺序 —— #
    #     if sort_ids is not None:
    #         unsort = [None] * len(scores)
    #         for new_i, old_i in enumerate(sort_ids):
    #             unsort[old_i] = scores[new_i]
    #         scores = unsort

    #     return scores

    def __call__(self, completions, src_text, tgt_text, lg, **kwargs):
        ...
        valid_indices = []  # 记录哪些 index 是有效的
        for idx, (completion, src, ref, lang) in enumerate(zip(completions, src_text, tgt_text, lg)):
            format_score = self.format([completion])[0]

            if format_score == 1.0:
                valid_indices.append(idx)
                match = re.search(r'<translate>(.*?)</translate>', completion, re.DOTALL)
                if match:
                    mt = match.group(1).strip()
                else:
                    print("format score is 1.0 but no <translate> tag found in completion")

                comets.append({"src": src, "mt": mt})
                refs.append(ref)
                mts.append(mt)
        ...
        # 默认 reward 都是 -3
        rewards = [-3.0] * len(completions)

        # 如果有有效数据才计算 BLEU 和 COMET
        if valid_indices:
            bleus = [self.bleu(ref, mt, lang) / 100.0 for ref, mt, lang in zip(refs, mts, tgt_langs)]

            # device = torch.device(f"cuda:{torch.cuda.current_device()}")
            # comet_scores = self.comet_score_no_trainer(
            #     comets, batch_size=8, device=device
            # )
            comet_scores = self.model.predict(comets, batch_size=8, gpus=0,progress_bar=False).scores
            # comet_scores = np.zeros(len(comets))

            bleus = np.array(bleus)
            comet_scores = np.array(comet_scores)

            final_scores = bleus + comet_scores

            for i, score in zip(valid_indices, final_scores):
                rewards[i] = float(score)  # 转为 float，确保类型一致
