...
def compute_score_single(data_source, solution_str, ground_truth, extra_info=None, return_separated=False):  
    """
    Single-item version of compute_score function for backward compatibility.
    Used by NaiveRewardManager.
    
    Args:
        return_separated: If True, returns a dict with separated rewards, otherwise returns combined score
    """
    ...
    if not format_score:  
        ...
        if return_separated:
            return {"R_tr": 0.0, "R_th": 0.0, "R_combined": -3.0}
    ...
    if think_text:
        ...
        thinking_result = call_api(src_text, think_text, src_lang_name, tgt_lang_name)
        thinking_score = extract_score_from_response(thinking_result)
    else:
        thinking_score = 0.0
    
    # Calculate separated rewards
    R_tr = (bleu_score / 100.0) + comet_score  # translate reward: bleu + comet
    R_th = thinking_score  # think reward: thinking score only
    R_combined = R_tr + R_th  # combined reward for backward compatibility
    
    if return_separated:
        print(f"Separated scores - R_tr: {R_tr:.3f}, R_th: {R_th:.3f}, R_combined: {R_combined:.3f}")
        return {"R_tr": R_tr, "R_th": R_th, "R_combined": R_combined}
    else:
        print("final score: ", R_combined)
        return R_combined
...
def compute_score(*args, **kwargs):
    ...
    # Check if this is positional arguments (single processing)
        ...
        return compute_score_single(data_source, solution_str, ground_truth, extra_info, return_separated)
...
def compute_score_batch(data_sources, solution_strs, ground_truths, extra_infos=None, micro_batch_size=8,return_separated=False):
    ...
    Args:
        ...
        return_separated: If True, returns dict with separated rewards lists
        
    Returns:
        List of final scores OR dict with separated rewards if return_separated=True
    """
    ...
    triplet_list = []
    final_scores = []
    # Initialize separated reward lists
    R_tr_scores = []  # translate rewards (bleu + comet)
    R_th_scores = []  # think rewards (thinking scores)
    R_combined_scores = []  # combined rewards
    ...
    if triplet_list:
        ...
        for i, (item, thinking_result) in enumerate(zip(triplet_list, thinking_results)):
            ...
            R_tr = (bleu_score / 100.0) + comet_score  # translate reward: bleu + comet
            R_th = thinking_score  # think reward: thinking score only
            R_combined = R_tr + R_th  # combined reward
            ...
            final_scores[original_index] = R_combined
            R_tr_scores[original_index] = R_tr
            R_th_scores[original_index] = R_th  
            R_combined_scores[original_index] = R_combined
            
            print(f"Item {original_index}: R_tr={R_tr:.3f}, R_th={R_th:.3f}, R_combined={R_combined:.3f} (format={format_score}, bleu={bleu_score/100.0:.3f}, comet={comet_score:.3f}, thinking={thinking_score:.3f})")
